...
class Gui:
    ...
    def __init__(self, desc, kwMap, matches, subcategoryList, categoryList, bucketList, keywordList):
        self.desc = desc
    ...
    def generateNewSubcatFrame(self):
        ...
        newSubcatLayout = [
            ...
            [sg.Listbox(self.bucketList, key='-BUCKET-', size = (20,len(self.bucketList)),select_mode="LISTBOX_SELECT_MODE_SINGLE")],
            [sg.T("Choose a class")],
            [sg.Listbox(["Expenditure","Income"], key='-CLASS-',size = (20,2),select_mode="LISTBOX_SELECT_MODE_SINGLE")]
        ]
    ...
    def getSubcategoryLoop(self):
        ...
        while True:
           ...
           # updating the search lists
            ...
            # show or hide KW Frame
            self.checkFrameVisibility(values['-NEW KW FLAG-'], '-NEW KW FRAME-')
            # show or hide the Subcat Frame
            self.checkFrameVisibility(values['-NEW SUBCAT FLAG-'], '-NEW SUBCAT FRAME-')
            # show or hide the Once-Off Frame
            self.checkFrameVisibility(values['-ONCEOFF FLAG-'], '-ONCEOFF FRAME-' )
            
            # disable/enable based on once-off checkbox
            if values['-ONCEOFF FLAG-']:
...
# Question for Bradley: I'm doing a lot of these things EVERY loop, but really I only need to update when things change. Should I care? I can implement it with enable events on components
